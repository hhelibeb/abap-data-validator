...
CLASS ltc_validation_check IMPLEMENTATION.
  ...
  METHOD deep_input.
    ...
    DATA: deep_table TYPE zcl_adata_validation=>ty_result_t.
    DATA: rules TYPE zcl_adata_validation=>ty_rules_t.
    DATA(adata_validation) = NEW zcl_adata_validation( ).
    TRY.
  ...
  METHOD error_input.
    ...
    rules = VALUE #(
      ( fname = 'FIELD1' required = abap_true                                user_type = zcl_adata_validation=>c_type_date )
      ( fname = 'FIELD2'                       initial_or_empty = abap_true  user_type = zcl_adata_validation=>c_type_date )
      ( fname = 'FIELD6' required = abap_true                                user_type = zcl_adata_validation=>c_type_email )
    ).
  ...
  METHOD regex_input.
    ...
    rules = VALUE #(
      ( fname = 'FIELD6' user_type = zcl_adata_validation=>c_type_email regex = 'gmail\.com$' regex_msg = 'Only gmail supported')
    ).
